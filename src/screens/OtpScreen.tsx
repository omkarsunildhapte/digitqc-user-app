...
export default function OtpScreen({ route, navigation }: Props) {
    ...
    const onVerify = () => {
        // Ensure all OTP digits are entered
        if (code.some((c) => c === "")) {
        ...
        const otp = code.join("");
        Keyboard.dismiss();
        // In real app: call verify API with otp
        toast.show("Welcome, TestUser!", "success");
        setTimeout(() => {
                ...
                <View className="flex-row justify-center gap-4 mb-6">
                    {code.map((c, i) => (
                        <TextInput
                            ...
                            maxLength={1}
                            className="w-14 h-14 border border-gray-200 rounded-xl text-center text-xl font-bold bg-gray-50 text-gray-900 focus:border-blue-500"
                            textAlign="center"
                        />
                    ))}
                </View>
